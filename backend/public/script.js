document.addEventListener('DOMContentLoaded', () => {
  ...
  const grammarModalTextarea = document.getElementById('grammarModalTextarea');
  const grammarModalBackdrop = document.getElementById('grammarModalBackdrop');
  const grammarModalSuggestions = document.getElementById('grammarModalSuggestions');
  const grammarModalCloseBtn = document.getElementById('grammarModalCloseBtn');
  const grammarModalApplyBtn = document.getElementById('grammarModalApplyBtn');
  const grammarModalFormatBtn = document.getElementById('grammarModalFormatBtn');
  let activeGrammarTextarea = null; // To store reference to the original textarea
  ...
  const API = {
    async getPatients(page, limit) {
      ...
      const r = await fetch(
        `/api/patients?page=${page}&limit=${limit}${searchQuery}${sortQuery}${filterQuery}`,
      );
    ...
    async processPdf(formData) {
      const r = await fetch('/api/extraction/upload', { method: 'POST', body: formData });
      if (!r.ok) {
        const e = await r.json();
        throw new Error(e.message || 'Failed to process PDF');
      }
      return r.json();
    },
    async getStats() {
    ...
    async checkGrammar(text) {
      ...
      if (!r.ok) throw new Error('Failed to check grammar');
      return r.json();
    },
  };
  ...
  loginForm.addEventListener('submit', (e) => {
    ...
    if (username === 'afp_boveda' && password === 'afp_demo') {
  ...
  patientDetailContainer.addEventListener('click', function (e) {
    ...
    const activeTabId =
      document.querySelector('#detailTabs .dashboard-tab.active')?.dataset.tab || 'summary';
    ...
    if (isEditMode && addBtn) {
      ...
      if (addBtnId === 'addConsultationBtn') {
        ...
        if (!selectedPatient.medical_encounters.lab_results)
          selectedPatient.medical_encounters.lab_results = [];
        selectedPatient.medical_encounters.lab_results.push({ results: [] });
      } else if (addBtnId === 'addRadiologyReportBtn') {
        if (!selectedPatient.medical_encounters.radiology_reports)
          selectedPatient.medical_encounters.radiology_reports = [];
    ...
    const grammarCheckBtn = e.target.closest('.grammar-check-btn');
    if (grammarCheckBtn) {
      activeGrammarTextarea = grammarCheckBtn.closest('.relative').querySelector('textarea');
      if (activeGrammarTextarea) {
        grammarModalTextarea.value = activeGrammarTextarea.value;
        grammarModal.classList.remove('hidden');
        handleGrammarCheck(); // Now checks text from the modal's textarea
      }
    }

    // --- Handle Clicking a Grammar Suggestion to Highlight Text ---
    const suggestionItem = e.target.closest('.grammar-suggestion-item');
    if (suggestionItem) {
      const offset = parseInt(suggestionItem.dataset.offset, 10);
      const length = parseInt(suggestionItem.dataset.length, 10);
      // The textarea is now always the one in the modal
      if (grammarModalTextarea && !isNaN(offset) && !isNaN(length)) {
        grammarModalTextarea.focus();
        grammarModalTextarea.setSelectionRange(offset, offset + length);
      }
    }
  });
  ...
  primaryBtn.addEventListener('click', () => {
    ...
    } else if (modalCurrentStep === modalTotalSteps) {
      // From Review to finish (Create Patient)
      uploadModal.classList.add('hidden');
      selectPatient(null); // Pass null to go to the detail view in create mode
      clearFile();
      modalCurrentStep = 1; // Reset for next time
    }
  });

  grammarModalCloseBtn.addEventListener('click', () => {
    grammarModal.classList.add('hidden');
    activeGrammarTextarea = null;
  });

  grammarModalFormatBtn.addEventListener('click', () => {
    const currentText = grammarModalTextarea.value;
    const formattedText = formatTextForReadability(currentText);
    grammarModalTextarea.value = formattedText;
    // Sync backdrop and automatically re-run the grammar check on the clean text
    grammarModalBackdrop.textContent = formattedText;
    handleGrammarCheck();
  });

  grammarModalTextarea.addEventListener('input', () => {
    // When user types, sync text to backdrop and clear highlights
    grammarModalBackdrop.textContent = grammarModalTextarea.value;
  });

  // Sync scrolling between textarea and backdrop
  grammarModalTextarea.addEventListener('scroll', () => {
    grammarModalBackdrop.scrollTop = grammarModalTextarea.scrollTop;
    grammarModalBackdrop.scrollLeft = grammarModalTextarea.scrollLeft;
  });

  grammarModalApplyBtn.addEventListener('click', () => {
    if (activeGrammarTextarea) {
      activeGrammarTextarea.value = grammarModalTextarea.value;
      // Trigger an input event to let any other listeners know the value has changed
      activeGrammarTextarea.dispatchEvent(new Event('input', { bubbles: true }));
    }
    grammarModal.classList.add('hidden');
    activeGrammarTextarea = null;
  });
  ...
  const updateModalUI = () => {
    ...
    switch (modalCurrentStep) {
      ...
      case 2:
        ...
        document.getElementById('summary-doc-type').textContent =
  ...
  function getPatientDataFromForm() {
    const data = selectedPatient.id ? {} : JSON.parse(JSON.stringify(selectedPatient));
    ...
    setValueByPath(data, 'patient_info.sex', checkedPatientSex ? checkedPatientSex.value : null);

    const checkedSponsorSex = document.querySelector('input[data-path="sponsor_info.sex"]:checked');
    setValueByPath(data, 'sponsor_info.sex', checkedSponsorSex ? checkedSponsorSex.value : null);
  ...
  async function handleSaveChanges() {
    ...
    try {
      ...
      if (selectedPatient.id) {
        // UPDATE
        const updatedData = getPatientDataFromForm();
        ...
        // CREATE
        const newPatientData = getPatientDataFromForm();
      ...
      if (error.response) {
        try {
          ...
          if (responseBody.message)
            detailedError = Array.isArray(responseBody.message)
  ...
  async function handleDeletePatient() {
    if (
      !selectedPatient?.id ||
      !confirm(
        `Are you sure you want to delete patient ${selectedPatient.name}? This cannot be undone.`,
      )
    ) {
      return;
    }
  ...
  async function handleDeleteSelected() {
    ...
    if (
      confirm(
        `Are you sure you want to delete ${selectedCount} patient record(s)? This cannot be undone.`,
      )
    ) {
      try {
  ...
  function updateSortUI() {
    ...
    sortOrderIcon.className =
      sortOrder === 'ASC' ? 'fa-solid fa-arrow-up-long' : 'fa-solid fa-arrow-down-long';
  ...
  window.changePage = (page) => {
    if (page > 0 && page <= Math.ceil(totalPatients / rowsPerPage)) fetchAndDisplayPatients(page);
  };
  ...
  function renderPatientDetails() {
    ...
    // Sync the height of the main content with the sidebar after rendering
    const sidebar = document.getElementById('detailSidebar');
    const mainDetailContent = document.getElementById('detailMain');

    if (sidebar && mainDetailContent) {
      // Use a timeout to ensure the DOM has been painted and heights are accurate
      setTimeout(() => {
        const sidebarHeight = sidebar.offsetHeight;
        // Set a minimum height for the main content, but allow it to be taller
        // if the sidebar is taller. 80vh is a reasonable default minimum.
        mainDetailContent.style.minHeight = `max(80vh, ${sidebarHeight}px)`;
      }, 0);
    }
  ...
  function renderDetailSidebar(patient) {
    ...
    const hasServiceInfo =
      patient_info.afpsn || patient_info.branch_of_service || patient_info.unit_assignment;
    ...
    if (isEditMode) {
      ...
      const patientMI = patient_info.full_name?.middle_initial
        ? `${patient_info.full_name.middle_initial}.`
        : '';
      const fullName = [
        patient_info.full_name?.first_name,
        patientMI,
        patient_info.full_name?.last_name,
      ]
        .filter(Boolean)
        .join(' ');
  ...
  function renderDetailMainContent(patient) {
    ...
    if (isEditMode) {
      ...
      summaryHTML = `<div class="space-y-6">
                                 <div>
                                    ...
                                    <p class="text-slate-700 mt-1 ml-7 leading-relaxed whitespace-pre-wrap">${markdownToHtml(summary?.key_findings?.value)}</p>
                                 </div>
                                 <div>
                                    <div class="flex items-center gap-3 text-lg font-semibold text-slate-700">
    ...
    if (isEditMode) {
      ...
      const sponsorFullName = [gn?.first_name, sponsorMI, gn?.last_name]
        .filter(Boolean)
        .join(' ')
        .trim();
    ...
    const fullName = [pi?.full_name?.first_name, patientMI, pi?.full_name?.last_name]
      .filter(Boolean)
      .join(' ')
      .trim();
  ...
  function createEditItem(label, value, path) {
    const isTextArea = [
      ...
      'key findings',
      'medications',
    ].includes(label.toLowerCase().replace(' / hpi', ''));
    let inputType = 'text';
    ...
    // Handle new object structure for fields with confidence scores
    const isObjectWithValue = typeof value === 'object' && value !== null && 'value' in value;
    const displayValue = isObjectWithValue ? value.value : value;
    const confidenceScore = isObjectWithValue ? value.confidence_score : null;
    ...
    const getConfidenceBadge = (score) => {
      ...
      return `<span class="confidence-badge ${colorClass}" title="AI Confidence: ${percentage}%">${percentage}%</span>`;
    };

    if (isTextArea) {
      return `<div class="py-2 relative">
                    <label class="block font-medium text-sm text-gray-600 mb-1">${label}</label>
                    ${getConfidenceBadge(confidenceScore)}
                    <textarea data-path="${path}.value" class="edit-textarea" spellcheck="true">${displayValue || ''}</textarea>
                    <button type="button" class="grammar-check-btn btn btn-secondary !py-1 !px-2 text-xs absolute right-1 bottom-3" title="Check Grammar & Spelling"><i class="fa-solid fa-spell-check"></i> Check</button>
                </div>`;
    }
    ...
    return `<div class="py-2 grid grid-cols-3 items-center gap-4">
                ...
                <input type="${inputType}" data-path="${isObjectWithValue ? path + '.value' : path}" value="${displayValue || ''}" class="edit-input col-span-2">
            </div>`;
  }
  function renderConsultations(consultations) {
    ...
    if (consultations && consultations.length > 0) {
      const quickLinks = consultations
        .map(
          (item, index) =>
                                ...
                                <div class="detail-card-header flex justify-between items-center" style="background-color: #f1f5f9;">
                                    ...
                                    ${
                                      isEditMode
                                        ...
                                        : item.age_at_visit
                                          ? `<span class="text-sm bg-blue-100 text-blue-800 font-medium px-2 py-0.5 rounded-full">Age: ${item.age_at_visit}</span>`
                                          : ''
                                    }
                                </div>
                                      ...
                                      isEditMode
                                        ...
                                        : `<div class="space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <div class="space-y-4">                                                    ${((icon, label, value) => `<div><div class="flex items-center gap-2 text-sm font-semibold text-slate-600"><i class="fa-solid ${icon} w-4 text-center"></i><span>${label}</span></div><p class="text-slate-700 mt-1 ml-6 whitespace-pre-wrap">${markdownToHtml(value?.value)}</p></div>`)('fa-comment-dots', 'Chief Complaint', item.chief_complaint)}
                                                    ${((icon, label, value) => `<div><div class="flex items-center gap-2 text-sm font-semibold text-slate-600"><i class="fa-solid ${icon} w-4 text-center"></i><span>${label}</span></div><p class="text-slate-700 mt-1 ml-6 whitespace-pre-wrap">${markdownToHtml(value?.value)}</p></div>`)('fa-file-lines', 'Notes / HPI', item.notes)}
                                                    ${((icon, label, value) => `<div><div class="flex items-center gap-2 text-sm font-semibold text-slate-600"><i class="fa-solid ${icon} w-4 text-center"></i><span>${label}</span></div><p class="text-slate-700 mt-1 ml-6 whitespace-pre-wrap">${markdownToHtml(value?.value)}</p></div>`)('fa-stethoscope', 'Diagnosis / Assessment', item.diagnosis)}
                                                    ${((icon, label, value) => `<div><div class="flex items-center gap-2 text-sm font-semibold text-slate-600"><i class="fa-solid ${icon} w-4 text-center"></i><span>${label}</span></div><p class="text-slate-700 mt-1 ml-6 whitespace-pre-wrap">${markdownToHtml(value?.value)}</p></div>`)('fa-prescription-bottle-medical', 'Treatment Plan', item.treatment_plan)}
                                      ...
                                      isEditMode
                                        ...
                                        : `<div class="space-y-2 text-sm">
                                            <div>
                                                ...
                                                <p class="text-slate-600 whitespace-pre-wrap">${markdownToHtml(report.findings?.value)}</p>
                                            </div>
                                            <div>
                                                <h4 class="font-semibold text-slate-700">Impression:</h4>
                                                <p class="font-medium text-slate-800 whitespace-pre-wrap">${markdownToHtml(report.impression?.value)}</p>
                                            </div>
                                        </div>`
                                    }
                                </div>
  ...
  async function handleGrammarCheck() {
    const text = grammarModalTextarea.value;
    const suggestionsContainer = grammarModalSuggestions;
    const backdrop = grammarModalBackdrop;

    if (!text.trim()) {
      suggestionsContainer.innerHTML =
        '<p class="text-sm text-slate-500 p-4 text-center">Nothing to check.</p>';
      return;
    }

    backdrop.innerHTML = ''; // Clear previous highlights
    suggestionsContainer.innerHTML =
      '<p class="text-sm text-slate-500 p-4 text-center animate-pulse">Checking...</p>';

    try {
      const data = await API.checkGrammar(text);
      if (data.matches.length === 0) {
        suggestionsContainer.innerHTML =
          '<p class="text-sm text-green-600 p-4 text-center"><i class="fa-solid fa-check-circle mr-1"></i> No issues found.</p>';
        backdrop.textContent = text; // Show plain text if no errors
      } else {
        // --- Render highlights in the backdrop ---
        let lastIndex = 0;
        const parts = [];
        data.matches.forEach((match) => {
          ...
          parts.push(
            `<mark class="grammar-error" data-match-id="${match.offset}">${errorText}</mark>`,
          );
        ...
        let resultsHTML = `<h4 class="text-sm font-semibold mb-2 text-slate-600">Suggestions (${data.matches.length})</h4>
                           <ul class="space-y-3">`;
        data.matches.forEach((match) => {
          const context = match.context.text;
          const highlightedContext = `${context.substring(0, match.context.offset)}<mark>${context.substring(match.context.offset, match.context.offset + match.context.length)}</mark>${context.substring(match.context.offset + match.context.length)}`;

          resultsHTML += `<li class="grammar-suggestion-item text-xs border-t border-slate-200 pt-3 cursor-pointer hover:bg-slate-100 -mx-3 px-3" data-offset="${match.offset}" data-length="${match.length}" data-match-id="${match.offset}">
                                        <p class="font-medium text-slate-700">${match.message}</p>
                                        <p class="my-1 text-slate-500 italic">...${highlightedContext}...</p>
                                        <p><strong class="text-slate-600">Suggestion:</strong> <span class="text-green-700 font-medium">${match.replacements[0]?.value || 'N/A'}</span></p>
                                      </li>`;
        });
        resultsHTML += '</ul>';
      ...
      backdrop.textContent = text; // Show plain text on error
      suggestionsContainer.innerHTML = `<p class="text-sm text-red-500 p-4 text-center">Error checking grammar. Please try again later.</p>`;
    }
  }
  ...
  function formatTextForReadability(text) {
    ...
    // 2. Normalize line breaks: ensure paragraphs are separated by a single newline
    // First, collapse any sequence of 2 or more newlines into a special marker.
    processedText = processedText.replace(/(\r\n|\r|\n){2,}/g, '::PARAGRAPH_BREAK::');
    // Then, replace any remaining single newlines with a space. This joins lines within a paragraph.
    processedText = processedText.replace(/(\r\n|\r|\n)/g, ' ');
    // Finally, restore the paragraph breaks.
    processedText = processedText.replace(/::PARAGRAPH_BREAK::/g, '\n\n').trim();

    // 3. Capitalize the first letter of each sentence.
    // This is a simple implementation that capitalizes after a period, question mark, or exclamation mark followed by a space.
  ...
  function populateCategoryFilter() {
    ...
    const categories = [
      ...new Set(allPatients.map((p) => p.patient_info?.category).filter(Boolean)),
    ];

    // Clear existing options and add the "All" option first
    categoryFilterMenu.innerHTML =
      '<a href="#" class="dropdown-item" data-value="">All Categories</a>';
